...
#windowed FST data
winTheta = pd.read_csv("selected_pops_windowed_10kbp_theta.csv")
...
merged = winTheta.merge(genes, on=["CHROM", "START", "END"], how="left")
merged["FBgn"] = merged["FBgn"].fillna("None")
merged["gene_symbol"] = merged["gene_symbol"].fillna("None")


#saving csv
merged.to_csv("selected_pops_windowed_10kbp_theta_mapped2genes.csv", index=False)