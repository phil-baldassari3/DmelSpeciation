...
#windowed FST data
winPi = pd.read_csv("selected_pops_windowed_10kbp_pi.csv")
...
merged = winPi.merge(genes, on=["CHROM", "START", "END"], how="left")
merged["FBgn"] = merged["FBgn"].fillna("None")
merged["gene_symbol"] = merged["gene_symbol"].fillna("None")


#saving csv
merged.to_csv("selected_pops_windowed_10kbp_pi_mapped2genes.csv", index=False)