...
#open data tables
fst = pd.read_csv("selected_pops_persite_fst.csv")
gene_map = pd.read_csv("biallelicSNPs_NoSingletons_missing0.05_dmel_r6_allChr_gene_ann.tsv", sep="\t", keep_default_na=False)
...
merged = fst.merge(gene_table, on=["CHROM", "POS"], how="left")
merged["FBgn"] = merged["FBgn"].fillna("None")
merged["gene_symbol"] = merged["gene_symbol"].fillna("None")

#saving mapped table
merged.to_csv("selected_pops_persite_fst_mapped2genes.csv", index=False)
